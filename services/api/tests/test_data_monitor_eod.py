...
def test_row_degrades_to_unknown_on_read_error():
    ...
    # a sym-backed bucket uses the injected (boom) sym conn (fx/rates/commodities/equity/universe AND
    # now index_levels open their own package DB, so they wouldn't hit the boom conn — fundamentals
    # stays in sym).
    bucket = next(b for b in BUCKETS if b.key == "fundamental")
