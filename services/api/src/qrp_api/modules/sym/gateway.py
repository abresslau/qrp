...
class DbSymGateway:
    ...
    def coverage_by_country(self, universe_id: str | None = None) -> list[dict]:
        """Population + per-layer coverage grouped BY COUNTRY (the world-map view). Same
        per-member-recency / index-bounded shape as universe_coverage() — never a
        count(DISTINCT) over prices_raw. `universe_id=None` aggregates the whole tracked
        population (distinct resolved members across all universes); otherwise one universe.
        Country/timezone come from exchange (joined via securities.mic). A country with
        several exchange timezones reports one representative (min) — enough to explain why
        its close differs from New York."""
        c = self._conn
        latest = _scalar(c, "SELECT max(session_date) FROM prices_raw")
        if latest is None:
            return []
        ...
        member_cte = (
            ...
            "WHERE resolution_status = 'resolved' AND universe_id = %(uni)s"
            if universe_id is not None
            else "SELECT DISTINCT composite_figi FROM universe_member_resolution "
            "WHERE resolution_status = 'resolved'"
        )
        rows = c.execute(
            f"""
            WITH members AS ({member_cte}),
            px AS (SELECT composite_figi, max(session_date) d FROM prices_raw
                    WHERE session_date >= %(latest)s - 14 GROUP BY composite_figi),
            rt AS (SELECT composite_figi, max(as_of_date) d FROM fact_returns
                    WHERE window_id = %(w)s AND as_of_date >= %(latest)s - 14 GROUP BY composite_figi),
            fn AS (SELECT composite_figi, max(as_of_date) d FROM fundamentals
                    WHERE as_of_date >= %(latest)s - 180 GROUP BY composite_figi)
            SELECT ex.country_iso, max(ex.country) AS country, min(ex.timezone) AS timezone,
                   count(*) AS members,
                   count(*) FILTER (WHERE s.status = 'active') AS active,
                   count(*) FILTER (WHERE s.status = 'active' AND px.d >= %(latest)s - 7) AS px_cov,
                   max(px.d) FILTER (WHERE s.status = 'active') AS px_latest,
              ...
              FROM members m
              JOIN securities s ON s.composite_figi = m.composite_figi
              JOIN exchange ex ON ex.mic = s.mic
              LEFT JOIN px ON px.composite_figi = m.composite_figi
              LEFT JOIN rt ON rt.composite_figi = m.composite_figi
              LEFT JOIN fn ON fn.composite_figi = m.composite_figi
             GROUP BY ex.country_iso
             ORDER BY members DESC, ex.country_iso
            """,
            {"latest": latest, "w": one_win, "uni": universe_id},
        ).fetchall()

        def _layer(cov: int, total: int, latest_d) -> dict:
            # `total` = ACTIVE members; delisted names are excluded from coverage entirely.
            status = "missing" if cov == 0 else "partial" if cov < total else "ok"
            return {
                "covered": cov,
                "total": total,
        ...
        return [
            ...
            for iso, country, tz, members, active, pxc, pxl, rtc, rtl, fnc, fnl in rows
        ]
