-- Deploy qrp_core:qrp_core to pg

BEGIN;

-- The qrp package's own database, holding the `qrp` schema: portfolios (weights-first, over
-- sym_id) + the Operate job ledger. Reads of sym (labels, returns) are assembled in-app; no FK
-- to sym (value-only composite_figi keys).
...
-- Operate job ledger: QRP-side trigger/status/exit for sym ops run as guarded background jobs.
