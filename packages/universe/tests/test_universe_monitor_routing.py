...
from universe import monitor as monitor_mod
from universe.gating import StageSummary, stage_and_promote
from universe.monitor import MONITOR_GATED, MONITOR_SUCCESS, run_monitor
...
class _Resolver:
    """No-op injected resolver: these tests assert gating/promotion routing, not resolution
    outcomes (the fake conn already serves the pending-tokens query)."""

    def resolve_fn(self, kind):
        return lambda toks: {}

    def local_resolve_fn(self):
        return lambda toks: {}

    def calendar_session(self, mic, on_or_before):
        return on_or_before
...
def test_snapshot_leaver_is_derived_and_staged(monkeypatch):
    ...
    s = run_monitor(conn, "ibov", _RES, as_of_date=D)
...
def test_confirm_and_reverse_rebuild_projection():
    from universe.gating import confirm_proposal, reverse_change
    ...
    conn = _ConfConn()
    assert confirm_proposal(conn, 7, _RES.local_resolve_fn()) is True
    assert ("ticker:B@BVMF", "leave") in conn.events
    assert conn.rebuilt == 1

    conn2 = _Conn()
    assert reverse_change(
        conn2, "ibov", "ticker:B@BVMF", "leave", D, _RES.local_resolve_fn()) is True
...
def test_reverse_refuses_to_reverse_a_corrective():
    ...
    with pytest.raises(UniverseError, match="join or leave"):
        reverse_change(_Conn(), "ibov", "ticker:B@BVMF", "correct", D, _RES.local_resolve_fn())
...
def test_provider_leave_and_derived_leave_count_once(monkeypatch):
    ...
    from universe.registry import EXACT
...
def test_rejected_change_resight_is_operator_only():
    ...
    from universe.gating import REASON_REJECTED_RESIGHT, is_promotable
