...
from universe.providers.index_source import ARCHETYPE_ETF, get_index_source
...
from universe.resolution import ResolveFn, Resolver
...
def _reference_as_figi_tokens(resolve_fn: ResolveFn, reference: set[str]) -> set[str]:
    """Resolve reference tokens to ``figi:`` tokens via the injected local (no-network) resolver.
    ...
    from the comparison.
    """
    resolved = resolve_fn(sorted(reference))
    out: set[str] = set()
...
def fetch_reference_tokens(
    ...
    """
    import universe.providers  # noqa: F401  (ensure archetype sources self-register)
    ...
    if not pref:
        from universe.providers.index_provider import DEFAULT_SOURCE_PREF
...
def run_configured_accuracy_check(
    ...
    *,
    ...
    if maintained and (
        ...
        reference = _reference_as_figi_tokens(resolver.local_resolve_fn(), reference)
