...
from universe.events import append_changes
...
def refresh_universe(
    ...
    *,
    ...
    ``conn`` is the universe DB; ``resolver`` is the injected sym-side identity bridge. Custom-list
    universes resolve against existing securities (no network); other kinds resolve via OpenFIGI
    (the resolver builds a default client).
    ...
    if calendar_mic and changes:
        from universe.monitor import align_changes
    ...
    # The injected resolver picks the strategy by kind (local-master for custom_list/criteria,
    # OpenFIGI for index) and owns the sym connection / client.
    resolve_fn = resolver.resolve_fn(kind)
