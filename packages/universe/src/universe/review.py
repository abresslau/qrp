...
from universe.accuracy import accuracy_alarms
from universe.gating import pending_proposals
from universe.monitor import DEFAULT_STALE_AFTER, stale_monitors
...
def build_digest(
    ...
    *,
    ...
    now: datetime | None = None,
    incomplete_members: dict[str, int] | None = None,
) -> Digest:
    """Assemble the operator review digest from every attention source.

    ``incomplete_members`` is the sym-side price-coverage summary (``completeness_summary``), passed
    in by the caller — universe is sym-import-free, so the sym data is injected, not imported.
    """
    return Digest(
