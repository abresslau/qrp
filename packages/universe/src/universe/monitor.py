...
from universe.gating import stage_and_promote
from universe.membership_diff import diff_identifier_sets
from universe.projection import MembershipEvent, pair_corrections, rebuild_projection
...
from universe.resolution import Resolver, resolve_universe_members
...
def align_changes(
    resolver: Resolver, changes: Iterable[MembershipChange], mic: str
) -> list[MembershipChange]:
    """Snap change effective dates to the ``mic`` exchange calendar (via the injected resolver)."""
    return snap_to_sessions(
        changes, lambda d: resolver.calendar_session(mic, d)
    )
...
def run_monitor(
    ...
    *,
    ...
    """
    import universe.providers  # noqa: F401  (ensure providers self-register)
    ...
    if as_of_date is None:
        ...
        if mic:
            as_of_date = resolver.calendar_session(mic, as_of_date) or as_of_date
    ...
    if calendar_mic:
        changes = align_changes(resolver, changes, calendar_mic)
    ...
    if promoted:
        resolve_universe_members(conn, universe_id, resolver.resolve_fn(kind))
