...
def test_first_name_is_inserted():
    ...
    assert write_name(conn, FIGI, "APPLE INC", as_of_date=date(2026, 6, 6)) == INSERTED
...
def test_unchanged_name_is_noop():
    ...
    assert write_name(conn, FIGI, "APPLE INC", as_of_date=date(2026, 6, 6)) == UNCHANGED
...
def test_same_day_correction_updates_in_place():
    ...
    assert write_name(conn, FIGI, "APPLE INC", as_of_date=date(2026, 6, 6)) == UPDATED
    sql = conn.sql()
...
def test_rename_on_a_later_day_closes_then_inserts():
    ...
    assert write_name(conn, FIGI, "META PLATFORMS INC-CLASS A", as_of_date=date(2022, 6, 9)) == REPLACED
    sql = conn.sql()
