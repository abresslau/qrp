...
def test_lcy_market_cap_is_price_times_shares():
    ...
    mc = market_cap(conn, conn, conn, "BBG", D)
...
def test_local_cap_then_converted_to_usd():
    ...
    lcy = market_cap(conn, conn, conn, "BBG", D)            # LCY
    usd = market_cap(conn, conn, conn, "BBG", D, "USD")     # restated
...
def test_missing_price_or_shares_yields_none_value():
    ...
    assert market_cap(no_px, no_px, no_px, "BBG", D).value is None
    assert market_cap(no_sh, no_sh, no_sh, "BBG", D).value is None
...
def test_missing_fx_leg_yields_none_value_but_keeps_inputs():
    ...
    mc = market_cap(conn, conn, conn, "BBG", D, "USD")
