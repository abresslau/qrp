...
def test_select_only_and_skip():
    assert [s.key for s in select_steps(only=["delta", "recompute"])] == ["delta", "recompute"]
...
def test_critical_failure_fails_run_but_isolates():
    def runner(key):
        if key == "delta":  # critical step fails
    ...
    delta = next(r for r in summary.results if r.key == "delta")
    assert delta.status == "error" and "vendor down" in delta.detail
