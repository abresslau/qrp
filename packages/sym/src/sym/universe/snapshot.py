...
A *snapshot pin* is ``(universe_id, as_of, log_version)`` where ``log_version`` is
...
def members_from_events(events: Iterable[MembershipEvent], as_of: date) -> set[str]:
    """The CompositeFIGI set that was a member on ``as_of`` given ``events`` (pure).

    Projects the events to intervals and selects FIGIs whose interval covers
    ``as_of`` (half-open ``[valid_from, valid_to)``).
    """
    members: set[str] = set()
    for figi, intervals in project_membership(events).items():
        for iv in intervals:
            if iv.valid_from <= as_of and (iv.valid_to is None or iv.valid_to > as_of):
                members.add(figi)
                break
    return members
...
def members_pinned(
    conn: psycopg.Connection, universe_id: str, as_of: date, log_version: int
) -> set[str]:
    """Members as-of ``as_of`` as the log stood at ``log_version`` (reproducible).
    ...
    return members_from_events(events, as_of)
