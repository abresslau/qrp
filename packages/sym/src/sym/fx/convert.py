...
``convert(amount, from_ccy, to_ccy, as_of)`` folds an amount between currencies as-of a
...
def triangulate(
    ...
    *,
    ...
    ``rate=1`` and ``observed_date=as_of``, so USD↔ccy works through the same formula.
    """
...
def convert(
    ...
    """Convert ``amount`` from ``from_ccy`` to ``to_ccy`` as-of ``as_of`` (None on any gap)."""
    amt = amount if isinstance(amount, Decimal) else Decimal(str(amount))
    if from_ccy == to_ccy:
        return amt  # identity — no rate lookup, never stale
    return triangulate(amt, fx_rate(conn, from_ccy, as_of), fx_rate(conn, to_ccy, as_of))
