...
def test_curve_sets_shapes_coverage_rows():
    conn = _Conn(sets_rows=[
        ("GB", "glc", "nominal", "spot", 15, date(2026, 6, 1), date(2026, 6, 19))
    ])
    out = DbRatesGateway(conn).curve_sets()
    assert out == [{
        "country": "GB", "curve_set": "glc", "basis": "nominal", "rate_type": "spot", "days": 15,
