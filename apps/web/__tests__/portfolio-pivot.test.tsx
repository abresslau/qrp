...
function holding(over: Record<string, unknown>) {
  return {
    ...
    window_returns: { "1D": 0, "1M": 0, "3M": 0, "6M": 0 }, freshness: "live",
    ...over,
  };
}
...
const COMP: Composition = {
  ...
  holdings: [
    holding({ figi: "F1", ticker: "AAPL", sector: "Tech", weight: 0.5, live_return: 0.1,
      window_returns: { "1D": 0.0123, "1M": 0.05, "3M": -0.02, "6M": null } }), // +4.17%
...
describe("PortfolioPivot", () => {
  it("renders explorer columns, sector groups, per-stock rows and a Daily P&L grand total", () => {
    ...
    for (const h of ["Ticker", "Country", "Exch", "Ccy", "Price", "Daily P&L", "MTD P&L", "YTD P&L", "Mkt cap", "Volume"]) {
  ...
  it("renders the trailing return columns then the Daily/MTD/YTD P&L contribution columns", () => {
    ...
    expect(headers.slice(i, i + 9)).toEqual([
      "Price", "1D Chg", "1M Return", "3M Return", "6M Return", "Daily P&L", "MTD P&L", "YTD P&L", "Mkt cap",
    ]);
    // the header row carries all 16 columns (5 explorer + Wt + Price + 4 windows + 3 P&L + MktCap + Vol)
    const headerCells = screen.getAllByRole("row")[0].querySelectorAll("th");
    expect(headerCells).toHaveLength(16);
    // AAPL row, by position: returns render re-based (null 6M -> —); Daily P&L = weight·live_return
    ...
    expect(cells[11]).toBe("+5.00%"); // Daily P&L = 0.5 × live_return 0.1
    expect(cells[12]).toBe("—"); // MTD P&L (no MTD return on F1)
  });
