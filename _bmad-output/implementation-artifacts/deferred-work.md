...
- **AC5 precedence-upgrade-closes-lower not implemented (design enhancement):** the chain is fill-only first-writer-wins (every source is fed only `read_unclassified_identities`), so a later higher-precedence/authoritative source can NEVER supersede an earlier lower-trust classification — most consequentially, a `source='llm'` (or stale) row is "sticky": when financedatabase/Yahoo later gain that name, they won't take over because the FIGI already has an effective `gics_scd` row. The SCD close-on-change machinery exists in `apply_classifications` but only fires for the SAME source's value changing, never for a cross-source precedence upgrade. AC5's literal "a higher-precedence source later filling a name closes the lower one (SCD)" is unbuilt. Fix would need a precedence-aware re-classification path (e.g. feed higher-precedence sources names whose current `source` ranks below them, and close+insert on win) — bigger than the fill-only MVP. Pairs with AC8's missing "source upgrade closes+inserts" test.
...
- **Pre-existing sym test breakage (NOT from this story; fails on clean HEAD) — INVOCATION-SPECIFIC:** `test_durable_reviews.py::test_fx_coverage_warns_on_open_rejections` does `from tests.test_fx_coverage import _Conn` with no `packages/sym/tests/__init__.py` → ModuleNotFoundError under `uv run pytest` from the package dir, yet 544/544 GREEN under `pytest tests -q` (C.1 auditor reproduced both) — the import resolves only when the rootdir/import-mode happens to put the package dir on sys.path. One-line fix (import the helper as the top-level `test_fx_coverage` module both invocations agree on); out of macro-story scope twice now.
