...
- **[RESOLVED 2026-06-29, retro action #4] Live header title block lacks `min-w-0`/`truncate`** (`apps/web/app/portfolios/[id]/live/page.tsx`) — a pathological long unbroken portfolio name could force horizontal header overflow (normal names wrap fine). FIXED with `min-w-0` on the flex ancestors + `break-words` on the `<h1>` (breaks the long token rather than truncating, so normal multi-word names still wrap — preferred over a hard `truncate` per the note's own "normal names wrap fine").
- **[RESOLVED 2026-06-29, retro action #4] Live `pct(null) → "—"` placeholder path uncovered in the strip suite** (`apps/web/__tests__/portfolio-pnl-strip.test.tsx`) — the strip renders with null Daily/MTD/YTD returns on every pre-composition-load. FIXED: added a test asserting each P&L stat (Daily/MTD/YTD) renders the `—` placeholder when its return is null.
...
## Deferred from: code review of optimiser-longshort-minvariance (2026-07-02)

- **Borrow drag uses the nominal short mass, not the priced-gross** (`packages/backtest/src/backtest/engine.py`, borrow-cost loop) — on a day where part of the short leg is unpriced, the strategy's daily return is rescaled to the priced gross, but the borrow charge is sized on the full nominal short mass (~0.5). Over-deducts financing on partial-pricing days. Second-order (borrow_bps defaults 0); charge on the priced short gross for exactness.
- **AC-6 smoke vol ratio is confounded by the alignment trim** (min_variance book trimmed 40/40→29/29 via the strict 252-day common-date alignment, `dropped_no_cov=968`) — a smaller dollar-neutral book carries lower vol partly from fewer names, so the headline 0.66 ratio overstates the pure covariance effect. The same-universe unit test (`test_min_variance_long_short_beats_inverse_vol_by_exploiting_covariance`) is the rigorous proof. A looser/pairwise covariance (so the book keeps its full 40/40) would give a like-for-like live comparison.
- **`_aligned_returns_asof` greedy trim is not max-cardinality** (`packages/backtest/src/backtest/engine.py`) — dropping the worst-covered name until ≥30 common dates does not find the largest ≥30-date feasible name subset, so the covariance may be built on fewer names than achievable. Acceptable v1; a smarter (biclustering / pairwise-covariance) approach is the upgrade.
